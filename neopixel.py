...
class Neopixel:
    ...
    # Function accepts (r, g, b) / (r, g, b, w) tuple
    def set_pixel(self, pixel_num, rgb_w, howBright = None):
        if howBright == None:
            howBright = self.brightness()
        pos = self.shift

        red = round(rgb_w[0] * (howBright / 255))
        green = round(rgb_w[1] * (howBright / 255))
        blue = round(rgb_w[2] * (howBright / 255))
        white = 0
        # if it's (r, g, b, w)
        if len(rgb_w) == 4 and 'W' in self.mode:
            white = round(rgb_w[3] * (howBright / 255))
